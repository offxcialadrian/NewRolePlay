...
public class Unarrest implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String s, String[] args) {
        ...
        if(!Beruf.getBeruf(p).equals(Beruf.Berufe.POLICE) && !Beruf.getAbteilung(p).equals(Abteilung.Abteilungen.JUSTIZMINSTERIUM)) {
            p.sendMessage(Messages.NO_PERMISSION);
            return true;
        }

        if(!Beruf.isLeader(p) && !Beruf.getAbteilung(p).equals(Abteilung.Abteilungen.JUSTIZMINSTERIUM)) {
