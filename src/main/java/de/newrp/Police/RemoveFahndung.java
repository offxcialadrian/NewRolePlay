...
public class RemoveFahndung implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String s, String[] args) {
        ...
        if (!SDuty.isSDuty(p)) {
            if (!(Beruf.getBeruf(p).equals(Beruf.Berufe.POLICE) || Beruf.getBeruf(p).equals(Beruf.Berufe.BUNDESKRIMINALAMT)) && !SDuty.isSDuty(p)) {
                p.sendMessage(Messages.NO_PERMISSION);
                return true;
            }
        ...
        if(Fahndung.getWanteds(tg) > 80) Beruf.Berufe.BUNDESKRIMINALAMT.sendMessage(Fahndung.PREFIX + "Die Fahndung von §6" + Script.getName(tg) + " §7wurde von §6" + Script.getName(p) + " §7entfernt.");
