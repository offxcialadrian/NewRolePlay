...
public class Straftat implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String s, String[] args) {
        ...
        if(Beruf.getAbteilung(p, true) != Abteilung.Abteilungen.JUSTIZMINISTERIUM) {
    ...
    public static void sendAllReason(Player p) {
        try (Statement stmt = NewRoleplayMain.getConnection().createStatement();
            ...
            if (rs.next()) {
                do {
                    p.sendMessage("§8» " + (Beruf.getBeruf(p) == Beruf.Berufe.GOVERNMENT && Beruf.getAbteilung(p, true) == Abteilung.Abteilungen.JUSTIZMINISTERIUM ? "§6§l#" + rs.getInt("id") + "§8: ":"") + "§6" + rs.getString("reason") + " §8(§6" + rs.getInt("amount") + " WantedPunkte§8)");
                } while (rs.next());
            }
        } catch (Exception e) {
            e.printStackTrace();
