...
public class StatsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender cs, Command cmd, String s, String[] args) {
        ...
        p.sendMessage("§7Beruf §8× §e" + (Beruf.hasBeruf(p) ? Beruf.getBeruf(p).getName() : "Kein Beruf") + (Beruf.isLeader(p, true)&&Beruf.hasBeruf(p) && Beruf.getAbteilung(p) != null ? " §8(§eLeader§8) " : " "+ (Beruf.getAbteilung(p) != null ? "§8(§e" + Beruf.getAbteilung(p).getName() + "§8)": "")) + (Beruf.hasBeruf(p)?" §8(§e" + Beruf.getSalary(p) + "€§8)":"Keiner"));
        p.sendMessage("§7Organisation §8× §e" + (Organisation.hasOrganisation(p) ? Organisation.getOrganisation(p).getName() + (Organisation.isLeader(p, true) && Organisation.hasOrganisation(p) ? " §8(§eLeader§8) " + "§8(§e" + Organisation.getSalary(p) + "€§8)" : " §8(§e" + Organisation.getRankName(p) + "§8) " + "§8(§e" + Organisation.getSalary(p) + "€§8)") : "Keine"));
